...
**Last updated:** 2026-05-31 (Phase 21K session closeout — handoff filed in `CHAT_CONTINUATION_2026-05-31.md`. **Current phase: 21K (complete).** **Last commit: `6d769d8`.** **Autoposter status: 🔴 DISABLED — Facebook token expired.** **Active blocker: Facebook Page Access Token expired 5/26/2026, needs refresh.** The autoposter cron auto-disabled itself (`autoposter_cron_enabled='false'`) on a `500` from Graph — "Session has expired on Tuesday, 26-May-26 21:00:00 PDT", platform=facebook, row `5f59a481-1858-4e4e-b7e4-58772d20f5bf`. FB App ID `2138194153633175`. Recovery: refresh the long-lived FB Page token → update `FACEBOOK_PAGE_ACCESS_TOKEN` in Vercel → redeploy → `UPDATE site_settings SET value='true' WHERE key='autoposter_cron_enabled';` → re-test. ⚠️ Do NOT create a new FB app (approved permissions took weeks). Phases shipped this session: 21E YouTube 20s→90s (CLIP_COUNT 4→18, `c39b1b5`), 21F MLM-language purge from SBA email templates (`2a76251`), 21G `contacts_status_check` widened — bounced/unsubscribed/rejected (`a5a1592`), 21H Resend bounce webhook auto-suppress (`6802099`), 21I TEMP DIAG logs removed from check-kling-jobs (`119cb25`), 21J send-sequences 2.5s inter-chunk delay — Resend 429 fix (`fb03874`), 21K stale "4 clips" comment cleanup in assembler (`6d769d8`). DB migrations 041/042/043 applied to production. TikTok row `4faa0732-9655-40cd-a7c7-3ff6ca7d7c9e` staged and ready, waiting on autoposter re-enable; TikTok token expires 6/1/2026 7:35 PM.)
**Last known good commit:** `6d769d8` — "Phase 21K: Update stale 4-clip comments in assembler to reflect dynamic clip count"
**Last updated:** 2026-05-30 (Phase 21 session closeout — the full cinematic YouTube pipeline is end-to-end automated and the first video is LIVE on YouTube: `https://www.youtube.com/watch?v=E28R9DG7yto`. Two distinct workstreams shipped this session. **Phase 20.2 — Instagram image proxy fix.** Operator's IG queue had been failing on `Status check failed: Authorization Error` for the container-status GET (row b3e6ce95 from the Phase 20 closeout). Root cause confirmed: Meta's container crawler cannot fetch raw `*.supabase.co` Storage URLs — the symptom was the same on every queued row, all 10 of which carried `mufpiphjddpacbxlbpqi.supabase.co/storage/v1/object/public/media/content/<file>.jpg`. Fix mirrors Phase 14AO/14AU's TikTok pattern: a `/v/i/:path*` Next.js rewrite proxies through the verified `www.vortextrips.com` host so Meta sees a known host and Vercel Edge streams from Supabase transparently. New `src/lib/instagram-image-proxy.ts` `rewriteImageUrlForInstagram()` helper. Three IG posting paths patched: manual `/api/automations/post-to-instagram` (commit `72321da`), cron `/api/cron/autoposter-once` (`fc994d5`), local runner `scripts/run-autoposter-once.js` (`fc994d5` — with its own JS mirror following the existing `validateMediaReadinessJs` / `proxyVideoUrlForTikTokJs` pattern). vortextrips.com and Pexels URLs pass through unchanged; unknown hosts log warn and pass. Row b3e6ce95 was unblocked by the fix (manual_posting_only stayed at default, but a re-trigger of the manual route succeeded once the proxy was deployed). **Phase 21A → 21D — Cinematic YouTube pipeline.** Four sequential phases built the full Sun-10:00-UTC → 12:00-UTC Monday automated YouTube video flow. **Phase 21A — ElevenLabs VO** (`24b2e57`): new `src/lib/elevenlabs.ts` HTTP-only TTS client, MP3 storage at `media/audio/vo/<uuid>.mp3`, smoke-test route `/api/admin/test-elevenlabs`. Migration `036_add_youtube_video_columns` adds five nullable TEXT columns (`elevenlabs_audio_url`, `kling_job_id`, `youtube_video_id`, `youtube_title`, `youtube_description`) + two partial indexes. `.env.example` documents `ELEVENLABS_API_KEY` + `ELEVENLABS_VOICE_ID` with Adam/Antoni/Rachel voice-id hints. **Phase 21B — Kling text-to-video** (`23e146b`, then re-platformed to PiAPI at `fec54eb`): initially built `src/lib/kling.ts` with hand-rolled HS256 JWT against `api-singapore.klingai.com` (`fa5d711` corrected base URL from `api.klingai.com`), then re-platformed onto PiAPI's reseller gateway (`api.piapi.ai`, `x-api-key` header, unified `/api/v1/task` endpoint with task_type='video_generation'). Verified against PiAPI's live docs before swapping — three divergences from the initial spec handled defensively: capitalized status strings (`Pending`/`Staged`/`Processing`/`Completed`/`Failed`), per-Kling-variant video URL paths (resource_without_watermark → resource → video), error detail at `data.error.message`. JWT helpers deleted entirely. Public function signatures (`generateCinematicClip`, `getKlingJobStatus`, `isKlingConfigured`) unchanged so all callers stay untouched. `/api/cron/check-kling-jobs` poller cron at `*/10 * * * *`, admin smoke-test at `/api/admin/test-kling`, migration `037_seed_kling_kill_switch`. **Phase 21C — Cinematic YouTube orchestrator** (`d0d53a7`): new `/api/cron/generate-youtube-video` Sunday-10-UTC cron (`maxDuration=300`), pipeline = pick-or-create YouTube row → runAIJob `video-script` jobType writes 90s script + 4 cinematic scene descriptions + title + description as STRICT JSON → ElevenLabs VO → Kling × 4 sequential submissions (per-clip failures land in JSONB as `status='failed'`, don't abort the row). Persists `elevenlabs_audio_url`, `youtube_title`, `youtube_description`, and `media_metadata.kling_clip_jobs[]` (4 entries, JSONB array — `kling_job_id` scalar column reserved for legacy single-clip rows). `check-kling-jobs` updated with multi-clip walk that handles both shapes per tick. Migration `038_seed_youtube_video_kill_switch`. Phase 21C deviated from the original plan on assembly (deferred to 21D), schema (JSONB array instead of new table), and AI router (runAIJob instead of direct gpt-4o). Migration `040_add_youtube_to_platform_check` (`ded263b`) added 'youtube' to the content_calendar platform CHECK constraint because the orchestrator's INSERT was being rejected by the old four-platform whitelist. **Phase 21D — Shotstack assembly + youtube-once trigger** (`3e28774`): replaced the FFmpeg-WASM-on-Vercel path proposed in 21C (rejected as too risky — 3-5× slowdown, 1.7GB memory ceiling, 250MB bundle limit) with Shotstack as a managed video API. New `src/lib/shotstack.ts` HTTP wrapper, `/api/cron/assemble-youtube-video` cron at `*/15 * * * *` (`maxDuration=120`) two-state machine (submit → poll). Re-hosts Shotstack's final MP4 to Supabase Storage at `content/youtube/<row_id>.mp4` so the URL is alive when `youtube-once` runs the next 12:00 UTC. Atomic UPDATE flips all five gate fields (`status='approved'`, `posting_status='ready'`, `posting_gate_approved=true`, `manual_posting_only=true`, `queued_for_posting_at=now()`). Migration `039_seed_assembly_kill_switch`. **Phase 21 fixes (8 follow-ups).** `1a2a32e` removed an empty-scalar-queue early return that was starving the multi-clip walk for YouTube rows. `5d71ccf` fixed three coordinated bugs in `check-kling-jobs` (PiAPI flips status=Completed before populating the video URL — clips were latching into terminal-completed-without-URL state; terminal-state guard now requires status='completed' AND has video_url; `rowMutated=false` no longer skips the all-done evaluation; demote 'completed without URL' to 'processing' so we re-poll). `634bdee` raised Shotstack speed clamp from 0.05 to 0.25 (Shotstack's documented minimum) — turned out to be wrong root cause. `9099d86` wrapped error logging in JSON.stringify so Vercel's depth=2 inspect cap stops collapsing the nested validation-error arrays to `[Array]`. `79d3e22` revealed the actual root cause: Shotstack rejects `speed` as an unknown property on clip objects (4 validation errors = 4 clips). Design swapped from 90s slow-motion to 20s highlight reel (native-speed clips back-to-back, VO trimmed to reel length). `7f5b58d` added `manual_posting_only: true` to the assembler's atomic gate flip (the column reads inverted from its name — `true` means "approved for autoposting"; was the last gap in the Phase 21A-D pipeline). `c6aa98c` surfaces `skipped_reasons` on the `youtube-once` `no_eligible_rows` response so future gate rejections are debuggable from a single curl. `9d612a8` corrected `validateMediaReadiness` step 3 — only fires "campaign media prompt exists but generated media is missing" when the platform ACTUALLY expects that media type (was over-firing on YouTube rows where `image_prompt` is destination metadata, on either_satisfies rows where the other media type covers the requirement, and on text-OK platforms). Also fixes the Check 2 false positive in the audit script. **First automated YouTube video shipped: `https://www.youtube.com/watch?v=E28R9DG7yto`.** Full pipeline (orchestrator → 4 Kling renders → Shotstack assembly → YouTube upload) ran end-to-end without operator intervention beyond the Vercel env-var setup. 9 of 40 Vercel cron slots used. tsc --noEmit clean across all 18 commits this session.)
**Last known good commit:** `9d612a8` — "fix(gate): correct media readiness check for youtube rows with video_url set"
...
## Phase 14AO — TikTok PULL_FROM_URL Verified-Domain Proxy (in working tree, 2026-05-10 — Next.js Edge rewrite + URL helper + 3 call-site updates; no DB schema change; no platform calls; no new dependencies)

### What this phase ships

A Next.js Edge rewrite at `/v/p/:path*` that proxies to Pexels' video CDN, plus a `proxyVideoUrlForTikTok()` helper called from all three TikTok publish paths to translate canonical `videos.pexels.com` URLs into the equivalent `www.vortextrips.com/v/p/...` URLs at publish time. TikTok now sees a verified-domain host on every `pull_from_url` request and accepts the publish init.

### Why this only surfaced today

A subtle layered failure that took two phases to fully unmask:

1. **Phase 14AN** revealed that the dashboard's "Upload to TikTok" button was a hardcoded `<a>` link to TikTok Studio — not an API call. Every previous click bypassed our integration entirely. The operator never saw a TikTok-side error because no API call ever fired from the dashboard.
2. **Phase 14AO** is what was hiding behind that bug: once the dashboard button actually called the API, the FIRST real response from TikTok was the URL-ownership rejection: `"TikTok publish init failed: Please review our URL ownership verification rules at https://developers.tiktok.com/doc/content-posting-api-media-transfer-guide/#pull_from_url"`. Phase 14AG removed video re-hosting to save cron time; today's rows have `video_url` on `videos.pexels.com` (Pexels' CDN — out of our control). We verified `www.vortextrips.com` for TikTok's domain ownership check; Pexels' domain is not verified and can't be (no `.well-known` access on their infrastructure).

The autoposter cron + manual runner script have been calling the same backend with the same Pexels URLs and presumably also hitting this error since Phase 14AG, but those paths run async / from terminal and don't surface errors as visibly to the operator. The dashboard manual button is what made the failure visible.

### Files touched

| File | Change |
|---|---|
| `next.config.js` | New `rewrites()` block. Single rule: `{ source: '/v/p/:path*', destination: 'https://videos.pexels.com/video-files/:path*' }`. Vercel's Edge handles the rewrite as a reverse proxy — TikTok hits `vortextrips.com/v/p/<id>/<file>.mp4`, Vercel transparently fetches from Pexels and streams the response back. No serverless function involvement (no function timeout risk). The rewrite preserves Range headers, content-type, content-length — everything TikTok needs for a chunked download. |
| `src/lib/tiktok-oauth.ts` | New exported `proxyVideoUrlForTikTok(videoUrl, appHostOverride?)` helper. Parses the URL, checks if it's `videos.pexels.com/video-files/...`, rewrites to `<APP_HOST>/v/p/<tail>` if so, returns unchanged otherwise. `APP_HOST` resolves to `appHostOverride` → `NEXT_PUBLIC_APP_URL` → `https://www.vortextrips.com` fallback (the canonical production host). Defensive: returns `videoUrl` unchanged on parse errors / non-string inputs / non-matching hosts. |
| `src/app/api/automations/post-to-tiktok/route.ts` | Imports `proxyVideoUrlForTikTok`. After the existing video_url validation, computes `videoUrl = proxyVideoUrlForTikTok(rawVideoUrl)` and uses that in `source_info.video_url`. The DB row's `video_url` is unchanged (still the canonical Pexels URL). |
| `src/app/api/cron/autoposter-once/route.ts` | Imports `proxyVideoUrlForTikTok`. New `proxiedVideoUrl = proxyVideoUrlForTikTok(post.video_url!)` line right before `initBody`. Same translation pattern. |
| `scripts/run-autoposter-once.js` | New JS-mirror function `proxyVideoUrlForTikTokJs(videoUrl, env)` matching the TS lib helper byte-for-byte. Used in the `postToTikTok` runner's `source_info.video_url`. |

### URL transformation example

```
Canonical (DB / dashboard):
  https://videos.pexels.com/video-files/12879953/12879953-uhd_2160_3840_24fps.mp4

Sent to TikTok:
  https://www.vortextrips.com/v/p/12879953/12879953-uhd_2160_3840_24fps.mp4
                                  ↓ Vercel Edge rewrite
  GET → https://videos.pexels.com/video-files/12879953/12879953-uhd_2160_3840_24fps.mp4
                                  ↓ stream the bytes back
  TikTok downloads the video, host is "www.vortextrips.com" → verified → accepts
```

### Why the rewrite is on Vercel Edge, not a serverless function

A serverless function streaming proxy would risk Vercel's 60s function timeout for long downloads — TikTok occasionally takes 30–60s to fully transfer a 30MB MP4, especially during peak load. `next.config.js` rewrites are handled by Vercel's Edge network layer, which has no such ceiling. Range headers and content-type pass through transparently, so TikTok's chunked download protocol works without modification.

### Domain verification compatibility

`www.vortextrips.com` is already verified per Phase 14AM (token file at `public/.well-known/tiktok-developers-site-verification.txt`). The proxy URL inherits that verification — TikTok only checks the host, not the path. No changes needed in the TikTok Developer Portal.

### What this phase does NOT do (deliberate scope cuts)

- ❌ No re-hosting of Pexels videos to Supabase Storage. The proxy approach is bandwidth-heavy on first publish but storage-cheap. Phase 14AG's "no re-hosting" decision still stands. If bandwidth becomes a concern, a future phase can add async re-hosting with cached results.
- ❌ No CSP update for the proxy path. The CSP `connect-src` already allows `'self'`, which covers `vortextrips.com/v/p/*`. The rewrite serves bytes outbound, not new connect-src destinations.
- ❌ No backfill of existing TikTok rows. Today's rows posted-via-the-broken-button never reached TikTok at all (they failed at the URL verification step that this phase fixes); they're still in the DB as approved + Ready and can be retried via the now-working button.
- ❌ No retry logic when TikTok returns a verification error. The dashboard's red toast already surfaces TikTok's error message verbatim; the operator can re-click after fixing any underlying issue. Adding automatic retry would be a separate phase.
- ❌ No symmetric proxy for Supabase Storage URLs. Old May 5–6 rows with `*.supabase.co` URLs won't go through the proxy. They were posted successfully under the old TikTok app's verification settings; those rows are also already `status='posted'` so they don't need to be re-published.

### Provider / platform / DB activity (this phase)

| Action | Count |
|---|---|
| HeyGen / Pexels / OpenAI / Facebook / Instagram / TikTok / X / email API calls | 0 (the proxy + helper only fire when the operator clicks Post to TikTok) |
| `UPDATE` / `INSERT` / `DELETE` against any DB table | 0 |
| `ALTER` / `CREATE` against any DB object | 0 |
| posted_at delta | 0 (30 → 30) |
| Net file change | 5 modified |

### Verification before commit

- ✅ `npm run lint` clean
- ✅ `npx tsc --noEmit` clean
- ✅ Helper handles all expected inputs: empty string, non-string, parse-failure, non-Pexels host, non-matching path → all pass through unchanged
- ✅ TS lib helper + JS mirror produce identical output for the same inputs (verified by manual trace)
- ✅ All three TikTok publish paths (manual route, cron, script) call the helper before sending to TikTok — verified by grep + diff

### Operator runbook to test

After Vercel deploys this commit (~1–2 min):

1. Hard-refresh `/dashboard/content`
2. Find a TikTok row with status `approved` + `Media ready`
3. Click "Mark Ready" → row flips to Ready state
4. Click "Post to TikTok"
5. **Expected:** green toast "Posted to TikTok!" — no more URL verification error
6. Wait ~60–90s, refresh `tiktok.com/@vortextrips` (or check the TikTok app on your phone)
7. New post should appear on profile (visible only to you under SELF_ONLY privacy in sandbox mode)

If step 5 still fails, paste the error message — TikTok may have additional checks (e.g., the Sandbox app's verified domains list might be separate from the production app's).
...
**14AO.1 — Symmetric proxy for legacy Supabase URLs (optional):** if you ever need to re-post one of the old May 5–6 rows, their video URLs would also fail TikTok's check. Same proxy pattern can be extended with a second rewrite rule covering `*.supabase.co/storage/v1/object/public/media/...`. Defer until needed.

**14AO.2 — Async re-hosting (optional):** for bandwidth control, a separate cron could re-host Pexels videos to Supabase Storage on a delay and update the DB. The rewrite path stays canonical; the underlying bytes shift to our infrastructure. Defer until bandwidth becomes a measurable concern.
