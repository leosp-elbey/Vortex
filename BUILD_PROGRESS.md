...
**Last updated:** 2026-05-30 (Phase 20.2 + Phases 21A-D session closeout. **First automated YouTube video LIVE: https://www.youtube.com/watch?v=E28R9DG7yto** — orchestrator → 4 Kling renders → Shotstack assembly → YouTube upload ran end-to-end with zero operator intervention beyond Vercel env var setup. **Phase 20.2 — IG image proxy.** `72321da` proxy Supabase image URLs through `vortextrips.com/v/i/*` for Meta's container crawler (new `next.config.js` rewrite + `src/lib/instagram-image-proxy.ts` helper; manual `/api/automations/post-to-instagram` patched). `fc994d5` patches the autoposter cron + JS runner script with the same proxy (3 IG posting paths now consistent). **Phase 21A — ElevenLabs voiceover pipeline.** `24b2e57` new `src/lib/elevenlabs.ts` HTTP-only TTS wrapper, MP3 storage at `media/audio/vo/<uuid>.mp3`, smoke-test route `/api/admin/test-elevenlabs`, migration `036_add_youtube_video_columns` (5 nullable TEXT columns + 2 partial indexes), `ELEVENLABS_API_KEY` + `ELEVENLABS_VOICE_ID` in `.env.example`. **Phase 21B — Kling text-to-video (via PiAPI).** `23e146b` initial direct-Kling client `src/lib/kling.ts` with hand-rolled HS256 JWT, `/api/cron/check-kling-jobs` poller at `*/10`, admin smoke-test, migration `037_seed_kling_kill_switch`. `fa5d711` corrected base URL to `api-singapore.klingai.com`. `fec54eb` re-platformed onto PiAPI's reseller gateway (`api.piapi.ai`, `x-api-key` header, `task_type='video_generation'` task envelope) — verified live docs first; defensive handling for capitalized status strings, per-Kling-variant video URL paths, `data.error.message` error envelope. JWT helpers deleted; public function signatures unchanged. **Phase 21C — Cinematic YouTube orchestrator.** `d0d53a7` `/api/cron/generate-youtube-video` Sunday 10:00 UTC (`maxDuration=300`) — pick-or-create YouTube row → runAIJob `video-script` writes 90s script + 4 scenes + title + description as JSON → ElevenLabs VO → 4 Kling submissions (per-clip failures isolated in JSONB). `check-kling-jobs` extended with multi-clip walk that handles both scalar `kling_job_id` and `media_metadata.kling_clip_jobs[]` JSONB shapes per tick. Migration `038_seed_youtube_video_kill_switch`. Deferred FFmpeg WASM to 21D (high risk on Vercel). `ded263b` migration `040_add_youtube_to_platform_check` added 'youtube' to the content_calendar platform CHECK constraint (orchestrator INSERTs were being rejected). **Phase 21D — Shotstack assembly + youtube-once trigger.** `3e28774` `src/lib/shotstack.ts` HTTP wrapper, `/api/cron/assemble-youtube-video` cron at `*/15` (`maxDuration=120`) two-state machine (submit → poll → re-host Shotstack MP4 to Supabase `content/youtube/<row_id>.mp4` → atomic UPDATE flips all 5 gate fields so existing `youtube-once` 12:00 UTC cron picks it up). Migration `039_seed_assembly_kill_switch`. **Phase 21 fixes (8 follow-ups, all this session):** `1a2a32e` fix(kling): run multi-clip walk even when scalar queue is empty (early-return was starving YouTube poller). `5d71ccf` fix(kling): debug and fix all-clips-done detection in multi-clip walk (PiAPI flips Completed before populating URL — clips were latching into terminal-completed-without-URL state; demote to processing + recover stale clips + compute allCompleted unconditionally). `634bdee` fix(shotstack): tighten speed clamp to [0.25, 1.0] (wrong root cause but kept). `9099d86` fix(shotstack): JSON.stringify the error logging so Vercel's depth=2 inspect cap stops collapsing validation arrays to [Array]. `79d3e22` fix(shotstack): remove invalid speed property (the actual root cause — Shotstack rejects speed on clip objects; design swapped from 90s slow-motion to 20s highlight reel). `7f5b58d` fix(youtube): set manual_posting_only=true in assembler gate flip (column reads inverted from its name; true = approved for autoposting; last gap in the pipeline). `c6aa98c` debug(youtube): surface skip_reasons in youtube-once response. `9d612a8` fix(gate): correct media readiness check — step 3 now respects platform rules, no longer false-positives on YouTube rows with `image_prompt` as destination metadata or on either_satisfies rows where the other media type covers the requirement. tsc --noEmit clean across all 18 commits. 9 of 40 Vercel cron slots used.)
**Earlier this session:** 2026-05-24 (Phases 19 + 20 session closeout. **Phase 19 — Caption overhaul: COMPLETE.** SOCIAL_SYSTEM rewritten to a HOOK → CONTRAST → PROOF → CTA template, deterministic `enforceCaptionRules` helper in `src/lib/caption-format.ts` guarantees the `vortextrips.com/free` link + 2-hashtag cap on every generator path, homepage savings claim standardized to "up to 75% off", all 114 active `content_calendar` rows regenerated via gpt-4o + the enforcer with rate-limit retry, 5s throttle, and idempotent skip. Commits: `dc2a4fb` (SOCIAL_SYSTEM rewrite), `60c5d19` (dashboard generator uses SOCIAL_SYSTEM), `15bc170` (enforcer + apply at all generator paths), `1b058b6` (homepage 75%), `a1e3e18` (backfill script dry-run default), `90092b9` (--limit N flag), `ad633b6` (rate-limit retry + throttle + idempotent skip), `d3ca4fb` (Phase 19 closeout doc). **Phase 20 — Autoposter stabilization: IN PROGRESS.** 20.0 SHIPPED at `84978be` — fix(autoposter): remove nonexistent description column + add kill-switch write error handling — `flipKillSwitchToDisabled()` was upserting a non-existent `site_settings.description` column, PostgREST rejected with 42703, the upsert's error was never destructured, and the kill switch silently stayed `true` after every "auto-disabled" log line. Now the field is removed and any future upsert error is surfaced via `console.warn`. 20.1 COMPLETE (read-only diagnostic) — Meta token health verified: both FB + IG tokens are the same permanent PAGE token (`expires_at: 0`, all required scopes, IG link intact, `/me` + `/{ig_id}/media` succeed). May 23 "Authorization Error" was NOT token expiry, revocation, or scope change. 20.2 IN PROGRESS — row `b3e6ce95-3640-4010-a45c-ee6290c9f146` failed 4 consecutive IG cron ticks with the same auth-shaped error from the container-status GET despite a healthy token and a publicly-fetchable Supabase Storage `image_url`. Suspected: Meta crawler cannot fetch from Supabase Storage from its IP range (or image fails an undocumented spec check). Mitigation: row marked `rejected`, autoposter re-enabled. Open blocker before fully trusting IG queue.)
**Earlier this session:** 2026-05-10 (Phase 14AO shipping in working tree — TikTok PULL_FROM_URL Verified-Domain Proxy. Once Phase 14AN wired the dashboard button to the actual API, the operator finally saw TikTok's real rejection: URL ownership verification failed because today's `video_url` is on `videos.pexels.com` (Pexels CDN — Phase 14AG removed re-hosting) and we can't put a `.well-known` verification file on Pexels' infrastructure. Fix: a Next.js Edge rewrite at `/v/p/:path*` proxies to Pexels' video CDN; new `proxyVideoUrlForTikTok()` helper translates the canonical Pexels URL into the proxy URL at publish time; called from manual post route + autoposter cron + runner script JS mirror. The DB still stores the canonical Pexels URL; only the URL sent to TikTok is rewritten so TikTok sees `vortextrips.com` (verified). Lint + typecheck clean.)
...
Found the latent bug while diagnosing why the operator's "Upload to TikTok" clicks weren't producing posts during the app review demo recording. The dashboard's TikTok button at `src/app/dashboard/content/page.tsx:417-426` was a hardcoded `<a href="https://www.tiktok.com/creator-center/upload">` link, NOT a call to the backend. Every click since Phase 14R bypassed the integration entirely and dumped the operator in TikTok Studio's manual upload UI. The backend itself (Phases 14R / 14V / 14AM / 14AM.1) was always wired correctly; the autoposter cron + manual runner script have been calling it successfully. Only the dashboard's manual button was broken.

**Built in 14AN:**
- [x] **`src/app/dashboard/content/page.tsx`.** New `postToTikTok(item)` handler mirroring `postToInstagram` / `postToFacebook` exactly: POSTs to `/api/automations/post-to-tiktok` with `{ content_id }`, optimistic state flip on success, success/error toast. The `<a href="https://www.tiktok.com/creator-center/upload">` element is replaced with `<button onClick={() => postToTikTok(item)}>`. Label changes from "Upload to TikTok" to "Post to TikTok" to match the FB/IG verb pattern.

**Why this slipped through:** the autoposter cron + manual runner script call the backend directly, so the integration "appeared to work" via those paths. Nobody verified the dashboard manual button actually fired the API. Today's diagnostic session traced the discrepancy to the placeholder `<a>` tag.

**Verification:**
- ✅ `npm run lint` clean
- ✅ `npx tsc --noEmit` clean
- ✅ Handler shape byte-equivalent to FB/IG (verified by source diff)

**Operator unblock:** after deploy, click "Post to TikTok" on a Ready row → API actually fires → TikTok publishes → post appears on profile in 60–90s. Demo recording is now possible.
...
- [~] **Phase 13 — Stability Layer** (code-side complete 2026-05-02, awaiting Leo follow-ups)
  ...
  - [ ] **Leo to do (optional):** prune Vercel of unused `TIKTOK_CLIENT_KEY`, `TIKTOK_CLIENT_SECRET`, `TWITTER_BEARER_TOKEN`, `TWITTER_CLIENT_ID`, `TWITTER_CLIENT_SECRET`, `STRIPE_*` (all declared but unused in current code).
